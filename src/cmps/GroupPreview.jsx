  ...
  return (
    <>
      <div className="group-title-flex">
        <input onBlur={() => handleGroupNameChange(groupTitle, group)} style={titleHead} className="task-input hov" type="text" value={groupTitle} onChange={(e) => setGroupTitle(e.target.value)} />

        <span className="arrow" onClick={(() => setExpanded(prev => !prev))}>{expanded ? '👇🏻' : '👉🏻'}</span>
      ...
      <section className="group-list">
        ...
        {expanded && (
          <>
            ...
            <div>
              <input className="add-input" style={{ borderLeft: `5px solid ${group?.color}` }} onBlur={() => handleAddTask(group, newTaskTitle)} type="text" placeholder="+Add Task" value={newTaskTitle} onChange={(e) => setNewTaskTitle(e.target.value)} />
