...
export function GroupTitle({
  ...
  useEffect(() => {
    ...
    else document.removeEventListener("mousedown", handleClickOutsideInput);
    return () =>
  ...
  function colorModalToggle() {
    colorModal ? closeModal(colorModalId) : openModal(colorModalId);
  }
  ...
  useEffect(() => {
    if (editModal)
      document.addEventListener("mousedown", handleClickOutsideEditModal);
    else document.removeEventListener("mousedown", handleClickOutsideEditModal);
    return () =>
      document.removeEventListener("mousedown", handleClickOutsideEditModal);
  }, [editModal]);
  ...
  function editModalToggle() {
    editModal ? closeModal(editModalId) : openModal(editModalId);
  }
  ...
  return (
    ...
    >
      <button
        ...
        style={{
          visibility: isHovered || editModal ? "visible" : "hidden",
      ...
      <span
        ...
        style={{
          transform:
            !isMiniGroup && expanded ? "rotate(90deg)" : "rotate(0deg)",
          color: group.color,
        }}
      >
      ...
      <EditGroupTitleModal
        ...
        toggleConfirmationModal={() =>
          toggleConfirmationModal(editModalId, "group", boardId, group.id)
        }
