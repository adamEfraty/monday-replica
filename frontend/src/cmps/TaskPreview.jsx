...
export function TaskPreview({
  ...
  const deleteTaskModal = openModals.some(
    (modalId) => modalId === "delete-" + id
  );
  ...
  function deleteModalToggle() {
    deleteTaskModal ? closeModal("delete-" + id) : openModal("delete-" + id);
  }

  // if user click outside delete modal close it
  function handleClickOutsideModal(event) {
    if (
      !deleteTaskModalRef.current.contains(event.target) &&
  ...
  return (
    ...
    >
      ...
      <section
        ...
        style={{
          ...style,
          gridTemplateColumns: `${labels
            .map((label) => `${label.width}px`)
            .join(" ")} auto`,
        }}
        key={`task-${task.id}`}
      >
        {labels.map((label) => {
          ...
          return (
            <section
              ...
              className={`grid-item ${label.type} ${
                label.type === "taskTitle" ? "stick-task" : ""
              }`}
            >
              <DynamicCmp
                listeners={listeners}
